...
use rusted_ca::domain::repository::account_command_repository::AccountCommandRepository;
use rusted_ca::domain::repository::account_query_repository::AccountQueryRepository;
...
async fn setup_test_db() -> Result<Arc<Pool<MySql>>, Box<dyn std::error::Error>> {
...
async fn test_account_crud_operations() {
    ...
    let pool = setup_test_db().await.expect("Failed to connect to database");
    ...
    // Test CREATE
    let create_result = repository.create(&account).await;
    assert!(create_result.is_ok(), "Failed to create account: {:?}", create_result);
    println!("✅ Account created successfully");
    
    // Test READ
    let retrieved_account = repository.find_by_id(&account_id).await.unwrap();
    assert!(retrieved_account.is_some(), "Account not found after creation");
    let retrieved = retrieved_account.unwrap();
    
    assert_eq!(retrieved.id(), account.id());
    assert_eq!(retrieved.merchant_name().value(), account.merchant_name().value());
    ...
    assert!(update_result.is_ok(), "Failed to update account: {:?}", update_result);
    ...
    let duplicate_result = repository.create(&duplicate_account).await;
    assert!(duplicate_result.is_err(), "Should fail to create duplicate email account");
...
async fn test_payment_database_operations() {
    ...
    let payment_repo = MySqlPaymentRepository::new(pool.clone());
    ...
    assert!(create_result.is_ok(), "Failed to create payment: {:?}", create_result);
    println!("✅ Payment created successfully");
    
    // Test payment READ
    let retrieved_payment = payment_repo.find_by_id(&payment_id).await.unwrap();
    assert!(retrieved_payment.is_some(), "Payment not found after creation");
    ...
    assert_eq!(retrieved.description(), Some(&"Database integration test payment".to_string()));
    assert_eq!(retrieved.customer_email(), Some(&"customer@example.com".to_string()));
    ...
    assert!(update_result.is_ok(), "Failed to update payment: {:?}", update_result);
    ...
    let successful_payments = payment_repo.find_by_status(&PaymentStatus::Success).await.unwrap();
...
async fn test_transaction_consistency() {
    ...
    for (i, amount_value) in payment_amounts.iter().enumerate() {
        ...
        // Process payment
        let mut retrieved_payment = payment_repo.find_by_id(payment.id()).await.unwrap().unwrap();
...
async fn test_error_handling_and_constraints() {
    ...
    assert!(invalid_payment_result.is_err(), "Payment with invalid account should fail");
    ...
    let non_existent_account = account_repo.find_by_id(&non_existent_account_id).await.unwrap();
    assert!(non_existent_account.is_none(), "Non-existent account should return None");
    
    let non_existent_payment_id = rusted_ca::domain::value_object::PaymentId::generate();
    let non_existent_payment = payment_repo.find_by_id(&non_existent_payment_id).await.unwrap();
    assert!(non_existent_payment.is_none(), "Non-existent payment should return None");
...
async fn test_concurrent_operations() {
    ...
    for i in 0..num_accounts {
        ...
        let handle = tokio::spawn(async move {
            ...
            let result = repo.create(&account).await;
    ...
    for (account_id, result) in results {
        assert!(result.is_ok(), "Concurrent account creation failed: {:?}", result);
