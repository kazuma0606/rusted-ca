...
impl RequestId {
    ...
    pub fn from_string(s: &str) -> Result<Self, DomainError> {
        Uuid::parse_str(s)
            .map(Self)
            .map_err(|_| DomainError::InvalidValue(format!("Invalid request ID: {}", s)))
    }
...
mod tests {
    ...
    fn test_request_id_from_string() {
        ...
        let request_id = RequestId::from_string(uuid_str).unwrap();
        assert_eq!(request_id.to_string(), uuid_str);
    }

    #[test]
    fn test_request_id_invalid_string() {
        let result = RequestId::from_string("invalid-uuid");
