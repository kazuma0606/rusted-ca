...
impl MySqlPaymentRepository {
    ...
    fn payment_to_params(&self, payment: &Payment) -> (
        String, // id
        String, // account_id
        i64,    // amount_cents
        ...
        let metadata_json = payment.metadata()
...
impl PaymentCommandRepository for MySqlPaymentRepository {
    async fn create(&self, payment: &Payment) -> DomainResult<()> {
        ...
        match result {
            ...
            Err(e) => Err(DomainError::InvalidOperation(format!("Database error: {}", e))),
        }
    }
    ...
    async fn update(&self, payment: &Payment) -> DomainResult<()> {
        ...
        let result = sqlx::query!(
            ...
            params.7, params.8, params.9, params.11, params.0
        )
        .execute(&*self.pool)
        .await;
            ...
            Ok(query_result) => {
                if query_result.rows_affected() == 0 {
                    Err(DomainError::InvalidOperation(format!(
                        "Payment with ID {} not found", payment.id()
                    )))
                } else {
                    Ok(())
                }
    ...
    async fn delete(&self, payment_id: &PaymentId) -> DomainResult<()> {
        ...
        match result {
            Ok(query_result) => {
                if query_result.rows_affected() == 0 {
                    Err(DomainError::InvalidOperation(format!(
                        "Payment with ID {} not found", payment_id
                    )))
                } else {
                    Ok(())
                }
...
impl PaymentQueryRepository for MySqlPaymentRepository {
    ...
    async fn search(&self, criteria: &PaymentSearchCriteria) -> DomainResult<Vec<Payment>> {
        ...
        Err(DomainError::InvalidOperation("Search not fully implemented yet".to_string()))
    }

    async fn count(&self, _criteria: &PaymentSearchCriteria) -> DomainResult<u64> {
        // Placeholder implementation
        Err(DomainError::InvalidOperation("Count not implemented yet".to_string()))
    }
    ...
    async fn get_payment_stats(&self, account_id: &AccountId) -> DomainResult<PaymentStats> {
        ...
        match result {
            Ok(row) => Ok(PaymentStats {
                total_payments: row.total_payments as u64,
                successful_payments: row.successful_payments as u64,
                failed_payments: row.failed_payments as u64,
                pending_payments: row.pending_payments as u64,
                total_amount_cents: row.total_amount_cents,
                successful_amount_cents: row.successful_amount_cents,
                average_amount_cents: row.average_amount_cents as i64,
